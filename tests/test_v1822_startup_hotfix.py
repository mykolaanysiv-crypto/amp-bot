...
ROUTE_DIRS = (ROOT / "app" / "web" / "routes", ROOT / "app" / "web" / "event_routes")
            ...
            for fn in [n for n in ast.walk(tree) if isinstance(n, (ast.FunctionDef, ast.AsyncFunctionDef))]:
                ...
                for arg, default in zip(args, defaults):
                    if arg.arg.startswith("_") and _is_fastapi_body_default(default):
                        bad.append(f"{path.relative_to(ROOT)}:{fn.lineno}:{fn.name}:{arg.arg}")
    assert not bad, "FastAPI/Pydantic v2 startup-risk body params: " + ", ".join(bad)


def test_event_scanner_has_no_csrf_body_parameter():
    source = (ROOT / "app" / "web" / "event_routes" / "operations.py").read_text(encoding="utf-8")
