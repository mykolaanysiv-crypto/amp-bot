...
RULE_SPECS: tuple[RuleSpec, ...] = (
    ...
    RuleSpec("events.checkin_open_before_minutes", "events", "Відкрити check-in до події", 60, 0, 1440, "хв", "За скільки хвилин до початку дозволити QR/check-in."),
    RuleSpec("events.checkin_close_after_minutes", "events", "Закрити check-in після старту", 360, 15, 1440, "хв", "Через скільки хвилин після початку закрити check-in та звичайне підтвердження attendance."),
    RuleSpec("events.feedback_delay_minutes", "events", "Feedback після події", 120, 0, 10080, "хв", "Затримка після підтвердженої участі перед feedback."),
    RuleSpec("events.waitlist_reservation_minutes", "events", "Резерв waitlist", 120, 5, 10080, "хв", "На скільки хвилин резервується місце після просування з черги."),
    ...
    RuleSpec("privacy.retention_days", "privacy", "Строк зберігання даних", 0, 0, 36500, "днів", "Політика строку зберігання. 0 = без автоматичного фізичного видалення історичних/audit даних."),
)
...
SECTIONS = {
    ...
    "events": ("📅 Події", "Нагадування, check-in/attendance window, feedback і резерв місця з waitlist."),
