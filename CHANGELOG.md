...
- Фото квестів зберігаються як staff-private media у категорії `quest_proofs`; повторна подача коректно замінює/очищає попереднє фото.
- Суперадмін може вручну підтвердити виконання квесту для `joined / returned / completed` без participant submission; причина обов’язкова, XP нараховується існуючим idempotent service, дія аудитується.
- Події отримали явні `starts_at` + `ends_at`; create/edit форми та Telegram admin flow збирають час завершення, а існуючі події міграція backfill-ить як `start + 2h`.
- Відмітка участі за замовчуванням доступна за 60 хв до початку та до 60 хв після завершення. Обидва значення змінюються у Web → «Налаштування».
- Feedback scheduler прив’язаний до `ends_at`: стандартно через 10 хв після завершення для фактичних учасників (`checked_in / attended`), параметр також редагується у Web.
- Динамічні event/quest блоки вебпанелі оновлюються фоновим polling кожні 5 секунд без повного reload; активні/змінені форми не перезаписуються.
- При редагуванні опублікованої події або квесту актуальні дані автоматично push-яться відповідним Telegram-учасникам — повторний `/start` не потрібен.
- Alembic head: `20260921_0013` (`events.ends_at`, `quest_participations.proof_photo_path`).

# v1.17.2.3 — Quick XP Builder, Survey Editor & Quest Card UX

- «⚡ Швидкі XP» отримали type-aware форми: при зміні типу показуються лише релевантні поля для мініквізу, відео, опитування або короткої відповіді.
- Мініквіз тепер підтримує багато питань; кожне має власні варіанти, правильну відповідь, порядок і окрему XP-нагороду. XP нараховується одразу за кожну правильну першу відповідь, а проходження можна продовжити з наступного невідповідженого питання.
- Додано таблиці `quick_xp_questions` і `quick_xp_answers` та Alembic `20260921_0012`; існуючі одно-питальні квізи автоматично переносяться у нову модель.
- Тижневий ліміт Quick XP змінено з 15 на 30 XP за замовчуванням і винесено в Web → «Налаштування» як `quick_xp.weekly_cap` (1–500 XP).
- Поле «Порядок = 100» прибрано з Quick XP UI; порядок карток тепер визначається рекомендацією та датою створення, а питання переставляються зрозумілими стрілками.
- В «Опитуваннях» назва й опис редагуються окремо від XP/дедлайну/аудиторії; кожне питання має окреме «✏️ Редагувати», підтримує зміну типу/варіантів/фото та переставляння вгору/вниз.
- Уніфіковано стиль кнопки «✏️ Редагувати» і посилено відступи/розкладку розгорнутих форм, щоб блоки не накладалися.
- Картки квестів вирівняно по висоті; медіаблоки мають однакову геометрію, а «👁 Детально» лишається основною дією за аналогією з подіями.
